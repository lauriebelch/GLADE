...
def AncestralGenome(node, species_tree, ortho_folder_path, gains, species_names):
...
def main(ortho_folder_path):
    ...
    for node in species_tree.traverse("postorder"):
        if node.is_leaf():
            continue
        AncestralGenome(node, species_tree, ortho_folder_path, gains, species_names)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reconstruct ancestral genomes (numeric mode)")
