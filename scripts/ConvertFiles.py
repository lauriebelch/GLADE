...
def Convert_Gene_Trees(Input, SpeciesDict, SequenceIDsDict):
    """
    Convert Resolved Gene Trees:
    """
    ...
    with open(tree_in) as infile, open(tree_out, "w") as outfile:
        for line in infile:
            line = line.strip()
            if not line:
                continue
...
def main(ortho_folder_path):
    ...
    Convert_Gene_Trees(ortho_folder_path, SpeciesDict, SequenceIDsDict)
