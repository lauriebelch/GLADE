...
def FindLossesAfterDuplications(gene_tree, species_tree, species_names):
    ...
    # get ready to store losses
    loss_records = []
    # traverse gene tree
    for node in gene_tree.traverse():
        if node.is_leaf(): continue # skip if node is leaf
        if hasattr(node, "duplication_label"):
            ...
            for leaf in node.children[0].get_leaves():
                match = next((sp for sp in species_names if leaf.name.startswith(sp + "_")), None)
                if match:
                    leaves1.add(match)
                else:
                    raise ValueError(f"No matching species found for leaf name '{leaf.name}' in child 0")
            for leaf in node.children[1].get_leaves():
                match = next((sp for sp in species_names if leaf.name.startswith(sp + "_")), None)
                if match:
                    leaves2.add(match)
                else:
                    raise ValueError(f"No matching species found for leaf name '{leaf.name}' in child 1")
            # Find the species node corresponding to the gene node
            species_node = None
            for record in mdf:
                if record['Gene Node'] == node.name:
                    species_node = record['Species Node']
                    break
            # Find the row in s_df where 'Node' matches the species_node
            for record in s_df:
                if record['Node'] == species_node:
                    child_info = record
            child1 = child_info['Child1']
            child2 = child_info['Child2']
            combined_children = child1 + child2
            # check if there are any combined children in one of leaves1 or leaves2,
            # but not the other
            gene_loss_info = IdentifyGeneLoss(leaves1, leaves2, combined_children)
            # Record gene loss details
            for lost_species in gene_loss_info['loss_to_child1']:
                loss_records.append({
                    'Focal Node': node.name,
                    'Lost Species': lost_species,
                    'Child Node': node.children[0].name,  # child where species is missing
                    'Species Node': species_node
                })
            for lost_species in gene_loss_info['loss_to_child2']:
                loss_records.append({
                    'Focal Node': node.name,
                    'Lost Species': lost_species,
                    'Child Node': node.children[1].name,  # child where species is missing
                    'Species Node': species_node
                })
    return loss_records
...
def main(ortho_folder_path):
    ...
    gene_trees = {}
    with open(os.path.join(ortho_folder_path, "Resolved_Gene_Trees/Resolved_Gene_Trees.txt"), 'r') as f:
        for line in f:
            if ':' in line:
    ...
    species_tree = ete3.Tree(os.path.join(ortho_folder_path, "Species_Tree/SpeciesTree_rooted_node_labels.txt"), quoted_node_names=True, format=1)
    ...
    parent_output_file = os.path.join(ortho_folder_path, "GainsLossDuplication", "node_parent_info.txt")
    ...
    os.makedirs(os.path.join(ortho_folder_path,"GainsLossDuplication/"), exist_ok=True)
    
    ListDictToTSV(duplications, os.path.join(ortho_folder_path, "GainsLossDuplication/Duplications.tsv"))
    ListDictToTSV(speciation_loss, os.path.join(ortho_folder_path, "GainsLossDuplication/Loss_speciation.tsv"))
    
    column_order = ['Gain Node', 'Parent Node', 'Orthogroup']
    ListDictToTSV(gains_list, os.path.join(ortho_folder_path, "GainsLossDuplication/Gains.tsv"), column_order)
    column_order = ['Orthogroup', 'Focal Node', 'Lost Species', 'Child Node', 'Species Node']
    #print(postduplication_loss)
    ListDictToTSV(postduplication_loss, os.path.join(ortho_folder_path, "GainsLossDuplication/Loss_postduplication.tsv"), column_order)
